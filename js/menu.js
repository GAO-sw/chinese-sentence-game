    ...
    fetch('lessons.json')
        ...
        .then(data => {
            ...
            data.forEach(level => {
                ...
                levelHeader.innerHTML = `<span class="lang-zh">${level.level_name.zh}</span><span class="lang-ru">${level.level_name.ru}</span>`;
                ...
                level.lessons.forEach(lesson => {
                    ...
                    link.innerHTML = `<span class="lang-zh">${lesson.title.zh}</span><span class="lang-ru">${lesson.title.ru}</span>`;
        ...
        .catch(error => {
            ...
            lessonListContainer.innerHTML = `
                <span class="lang-zh">加载课程列表失败，请检查 lessons.json 文件是否存在且格式正确。(错误: ${error.message})</span>
                <span class="lang-ru">Не удалось загрузить список уроков. Проверьте, существует ли файл lessons.json и правильный ли у него формат. (Ошибка: ${error.message})</span>
            `;
        });
});
